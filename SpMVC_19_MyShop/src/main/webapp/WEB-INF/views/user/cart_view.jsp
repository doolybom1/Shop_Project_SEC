...
$(function(){
	...
	// replace는 뒤로가기 막을 때 사용하는 방법 중 하나
	...
	$(".btn_list_delete").click(function(){
		...
		if(confirm("선택된 상품을 삭제할까요?")) {
			// 배열이기때문에 ajax로 삭제하는게 편할듯
			// POST로 넘겨야 하기 때문에
			// "${_csrf.parameterName}" : "${_csrf.token}" 복사해오기
			
			// $.ajaxSettings.tranditional = true
			// 를 쓰면 컨트롤러에서 []를 안 써도 된다.
...
<div>
	<button data-check="1" class="btn_list_all_check">전체 선택하기</button>
