...
/*
 * STOMP를 지원하는 기능을 사용할 수 있도록 만들어져 있는 기본 class를 상속받아서 사용함
 */
@Slf4j
@Component
public class WebSocketController extends TextWebSocketHandler{

	// socket으로 서버에 접속할때 접속되는 client들을 관리하기 위한 session
	...
	// client가 웹소켓을 통해서 접속을 시도할때 처음 실행될 메서드 
	@Override
	...
	// client가 메시지를 보내면 메시지를 수신하는 컨트롤러 메서드
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		for(WebSocketSession ws : sessionList) {
			...
			// 문자열이나, 여러 메시지를 socket을 통해서 전송하기 편리한 STOMP 데이터 구조로 변경한다
